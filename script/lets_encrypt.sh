...
rm /root/.acme.sh/${MYDOMAIN}_ecc/fullchain.cer
rm /root/.acme.sh/${MYDOMAIN}_ecc/${MYDOMAIN}.key
...
cd ${SCRIPT_PATH}/sources/acme.sh/
bash acme.sh --issue --standalone --debug 2 --log -d ${MYDOMAIN} -d www.${MYDOMAIN} --keylength ec-384 --force-renew --staging >>"${main_log}" 2>>"${err_log}"
...
rm /root/.acme.sh/mail.${MYDOMAIN}/fullchain.cer
rm /root/.acme.sh/mail.${MYDOMAIN}/mail.${MYDOMAIN}.key
rm /etc/nginx/ssl/mail.${MYDOMAIN}.cer
rm /etc/nginx/ssl/mail.${MYDOMAIN}.key

cd ${SCRIPT_PATH}/sources/acme.sh/
bash acme.sh --issue --debug 2 --standalone -d mail.${MYDOMAIN} -d imap.${MYDOMAIN} -d smtp.${MYDOMAIN} --keylength 4096 --force-renew --staging >>"${main_log}" 2>>"${err_log}"
